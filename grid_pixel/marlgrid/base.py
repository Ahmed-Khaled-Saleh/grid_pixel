"""Base class for MARLGrid environments."""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../../nbs/00d_marlgrid.base.ipynb.

# %% auto 0
__all__ = ['TILE_PIXELS', 'ObjectRegistry', 'rotate_grid', 'MarlGrid', 'MarlGridEnv', 'get_goal_observation_before_reaching']

# %% ../../nbs/00d_marlgrid.base.ipynb 3
from fastcore import *
from fastcore.utils import *

# %% ../../nbs/00d_marlgrid.base.ipynb 4
"""Multi-agent grid world.
Based on MiniGrid: https://github.com/Farama-Foundation/Minigrid.
"""
...
# %% ../../nbs/00d_marlgrid.base.ipynb 11
@patch
def put_obj(self: MarlGridEnv, obj, i, j):
    """
...
@patch
def try_place_obj(self: MarlGridEnv, obj, pos):
...
@patch
def place_obj(self: MarlGridEnv, obj, top=(0, 0), size=None, reject_fn=None,
                max_tries=1e5):
...
# %% ../../nbs/00d_marlgrid.base.ipynb 13
@patch

def step(self: MarlGridEnv, action_dict):
...
# %% ../../nbs/00d_marlgrid.base.ipynb 15
@patch
def render(
        self: MarlGridEnv,
        mode='human',
    ...
    if mode == 'human' and not self.window:
        from grid_pixel.marlgrid.renderer import SimpleImageViewer
...
# %% ../../nbs/00d_marlgrid.base.ipynb 17
def get_goal_observation_before_reaching(env, agent, goal_obj):
    ...
    gx, gy = goal_obj
    # 1) Copy original agent state (safe)
    old_pos = agent.pos.copy()
    old_dir = agent.dir

    # 2) Compute forward direction vector
    ...
    fv = dir_vecs[agent.dir]

    # 3) Compute the position one step BEFORE reaching the goal
    # So: agent is placed at (goal_pos - fv)
    goal_pre_pos =  np.array((gx, gy)) - fv

    # 4) Temporarily move agent
    # Remove agent from grid
    old_cell = env.env.grid.get(*old_pos)
    if old_cell == agent:
        env.env.grid.set(*old_pos, None)
    else:
        old_cell.agents.remove(agent)

    # Place agent in pre-goal position
    env.env.grid.set(*goal_pre_pos, agent)
    agent.pos = goal_pre_pos

    # 5) Render observation
    obs = env.env.gen_agent_obs(agent, image_only=True)

    # 6) Restore agent back to its original location
    # Remove from new location
    cur_cell = env.env.grid.get(*goal_pre_pos)
    if cur_cell == agent:
        env.env.grid.set(*goal_pre_pos, None)
    else:
        cur_cell.agents.remove(agent)

    # Restore original location
    env.env.grid.set(*old_pos, agent)
    agent.pos = old_pos
    agent.dir = old_dir
